...
# Function to generate a complex graph showing actual code changes
 # Function to generate a complex graph showing actual code changes with concise labels and proper label positioning
def generate_complex_code_change_graph(code_changes, bug_overview, mitigations):
    # Create a directed graph using NetworkX
    G = nx.DiGraph()

    # Add nodes with concise summaries
    G.add_node("Code Change 1", label="Code Change 1: Sanitized Query")
    G.add_node("Bug Overview 1", label="Bug Overview 1: SQL Injection")
    G.add_node("Mitigation 1", label="Mitigation 1: Input Validation")
    G.add_node("Code Change 2", label="Code Change 2: Improved Input Handling")
    G.add_node("Bug Overview 2", label="Bug Overview 2: Security Risks")
    G.add_node("Mitigation 2", label="Mitigation 2: Parameterized Queries")
    ...
    pos = nx.spring_layout(G, k=2.0, seed=42)  # Spacing out nodes
    plt.figure(figsize=(12, 10))  # Adjust figure size
    
    # Draw the graph with customized settings
    nx.draw(G, pos, with_labels=False, node_color='#76c7c0', node_size=5000, font_size=12, font_weight='bold', arrows=True, edge_color='#ff6b6b')
    
    # Manually adjust label positions to show them below the nodes
    label_pos = {key: (value[0], value[1] - 0.15) for key, value in pos.items()}  # Adjust label positions downwards
    
    # Add labels with proper positioning below the nodes
    labels = nx.get_node_attributes(G, 'label')
    nx.draw_networkx_labels(G, label_pos, labels, font_size=10, font_color='#2f3136', verticalalignment='center', horizontalalignment='center')
